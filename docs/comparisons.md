...
- Termyte is the most local-first and SQLite-centered of the four.
...
|---|---|---|---|
| Termyte | Local persistent memory layer for coding agents | Durable capture, provenance, retrieval, and operator visibility in one small runtime | Narrower scope and smaller surface area |
...
It is trying to be the clearest local memory layer for coding agents:

- traces first
- durable queue second
- memory third
- retrieval and context injection after that
...
If you want a compact, local, inspectable memory layer that stays close to one repo and one SQLite database, Termyte is the sharper fit.
