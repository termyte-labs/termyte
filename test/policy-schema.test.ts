...
`;
  ...
  it("accepts supported policy modes", () => {
    const document = normalizePhaseOnePolicyDocument(parsePhaseOnePolicyYaml("version: 1\nmode: strict\npresets: []\nrules: []\n"), presets);
...
`;
  ...
  it("rejects unknown fields, versions, actions, presets, and empty matchers", () => {
    ...
    expect(validateRawPolicyDocument(parsePhaseOnePolicyYaml("version: 1\nmode: locked\npresets: []\nrules: []\n"), presets)).toContain("mode must be one of off, observe, standard, strict, paranoid");
