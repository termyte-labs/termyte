...
function Memories() {
  ...
  return <><Header eyebrow="ACCUMULATED CONTEXT" title="Memories" copy="Reusable knowledge derived from experience, with lifecycle, evidence, and correction history." /><section className="split"><div className="panel list">{memories.map(m => <button key={m.id} onClick={async () => setDetail(await api(`/api/memories/${m.id}`))}><div><b>{m.title}</b><span>{m.type}</span></div><Status value={m.lifecycle_state ?? m.state} /></button>)}</div><div className="panel detail">{detail ? <><small>{detail.memory.type}</small><h2>{detail.memory.title}</h2><p>{detail.memory.description}</p><div className="actions"><button onClick={() => feedback("helpful")}>Helpful</button><button onClick={() => feedback("ignored")}>Ignored</button><button className="danger" onClick={() => feedback("harmful")}>Harmful</button></div><h3>Applicability</h3><p className="mono">{detail.memory.lifecycle_state} · {detail.explanation.provenance_valid ? "valid provenance" : "broken or missing provenance"}</p><h3>Evidence</h3>{detail.explanation.source_evidence.map((e: any) => <pre key={e.id}><span>{e.kind}</span>{e.content}</pre>)}{detail.explanation.missing_evidence_ids.map((id: string) => <Notice key={id} text={`Broken evidence link: ${id}`} />)}<h3>Correction chain</h3>{detail.explanation.edges.map((e: any) => <div className="candidate" key={e.id}><span>{e.edge_type}</span><small>{e.peer_memory?.title ?? "missing memory"}</small></div>)}<h3>Feedback</h3>{detail.explanation.feedback.map((f: any) => <div className="candidate" key={f.id}><Status value={f.event_type} /><span>{f.source}</span></div>)}</> : <Empty text="Select a memory to inspect its provenance." />}</div></section></>;
}
