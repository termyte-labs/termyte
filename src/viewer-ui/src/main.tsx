...
function Overview() {
  ...
  return <><Header eyebrow="SYSTEM OVERVIEW" title="Agent experience, made legible." copy="Termyte observes work quietly and shows the context your coding agents carry forward." />
    ...
    <section className="panel"><h2>Context effects · last 30 days</h2><div className="candidate"><Status value="helped" /><span>{value?.effects?.helped ?? 0} helped</span><small>{Math.round((value?.effects?.helpfulRate ?? 0) * 100)}%</small></div><div className="candidate"><Status value="hurt" /><span>{value?.effects?.hurt ?? 0} hurt</span><small>{Math.round((value?.effects?.harmfulRate ?? 0) * 100)}%</small></div><div className="candidate"><span>Attribution coverage</span><small>{Math.round((value?.effects?.attributionRate ?? 0) * 100)}%</small></div></section>
...
function Memories() {
  ...
  return <><Header eyebrow="ACCUMULATED CONTEXT" title="Memories" copy="Reusable knowledge derived from experience, with lifecycle, evidence, and correction history." /><section className="split"><div className="panel list">{memories.map(m => <button key={m.id} onClick={async () => setDetail(await api(`/api/memories/${m.id}`))}><div><b>{m.title}</b><span>{m.type}</span></div><Status value={m.lifecycle_state ?? m.state} /></button>)}</div><div className="panel detail">{detail ? <><small>{detail.memory.type}</small><h2>{detail.memory.title}</h2><p>{detail.memory.description}</p><div className="actions"><button onClick={() => feedback("helpful")}>Helpful</button><button onClick={() => feedback("ignored")}>Ignored</button><button className="danger" onClick={() => feedback("harmful")}>Harmful</button></div><h3>Applicability</h3><p className="mono">{detail.memory.lifecycle_state} · {detail.explanation.provenance_valid ? "valid provenance" : "broken or missing provenance"}</p><h3>Evidence</h3>{detail.explanation.source_evidence.map((e: any) => <pre key={e.id}><span>{e.kind}</span>{e.content}</pre>)}{detail.explanation.source_observations.map((o: any) => <pre key={`observation:${o.id}`}><span>observation:{o.id}</span>{o.title}{o.description ? `\n${o.description}` : ""}</pre>)}{detail.explanation.source_traces.map((t: any) => <pre key={`trace:${t.id}`}><span>trace:{t.id}</span>{t.tool_name ?? t.event_type}</pre>)}{detail.explanation.missing_evidence_ids.map((id: string) => <Notice key={id} text={`Broken evidence link: ${id}`} />)}{detail.explanation.missing_source_observation_ids.map((id: number) => <Notice key={`observation:${id}`} text={`Broken observation link: ${id}`} />)}{detail.explanation.missing_source_trace_ids.map((id: number) => <Notice key={`trace:${id}`} text={`Broken trace link: ${id}`} />)}<h3>Correction chain</h3>{detail.explanation.edges.map((e: any) => <div className="candidate" key={e.id}><span>{e.edge_type}</span><small>{e.peer_memory?.title ?? "missing memory"}</small></div>)}<h3>Context effects</h3>{detail.explanation.context_effects.map((e: any) => <div className="candidate" key={e.id}><Status value={e.verdict} /><span>{e.injection_id}</span><small>{Math.round(e.confidence * 100)}%</small></div>)}<h3>Feedback</h3>{detail.explanation.feedback.map((f: any) => <div className="candidate" key={f.id}><Status value={f.event_type} /><span>{f.source}</span></div>)}</> : <Empty text="Select a memory to inspect its provenance." />}</div></section></>;
}

function Diagnostics() {
  const [value, setValue] = useState<any>(); useEffect(() => { api("/api/diagnostics").then(setValue); }, []);
  return <><Header eyebrow="LOCAL RUNTIME" title="Diagnostics" copy="Capture, durable processing, and failures remain invisible during agent work and inspectable here." /><section className="metrics"><article><small>ready</small><strong>{value?.health.queue.ready ?? "—"}</strong></article><article><small>oldest ms</small><strong>{value?.health.queue.oldestReadyAgeMs ?? "—"}</strong></article><article><small>attributed</small><strong>{Math.round((value?.effects?.attributionRate ?? 0) * 100)}%</strong></article><article><small>hurt</small><strong>{value?.effects?.hurt ?? "—"}</strong></article><article><small>dead</small><strong>{value?.health.queue.dead ?? "—"}</strong></article></section><section className="panel"><h2>Problem work</h2>{value?.problemJobs.length ? value.problemJobs.map((j: any) => <pre key={j.id}>{j.state} · {j.kind} · {j.last_error}</pre>) : <Empty text="No failed or dead-lettered work." />}<h2>Audit history</h2>{value?.audit.slice(0, 20).map((a: any) => <div className="candidate" key={a.id}><span>{a.operation}</span><small>{a.timestamp}</small></div>)}</section></>;
}
